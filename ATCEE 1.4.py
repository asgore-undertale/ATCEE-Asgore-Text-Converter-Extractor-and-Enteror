...
from ConvertingScripts.Fit_in_box import fit
...
def convert(text):
    ...
    if Windows.FIB_check.isChecked():#Fit in box
        if cell_bytes._textzone_width != '' and cell_bytes._textzone_lines != '':
            text = fit(text, fit_database, int(cell_bytes._textzone_width), int(cell_bytes._textzone_lines), cell_bytes._line_command, cell_bytes._page_command, cell_bytes._start_command, cell_bytes._end_command)
        else:
