...
#نافذة معلومات مهمة
ImportantWindow = QMainWindow()
ImportantWindow.setFixedSize(378, 60)
ImportantWindow.setWindowTitle("معلومات مهمة")

important_textbox = QTextEdit(ImportantWindow)
important_textbox.setGeometry(QtCore.QRect(0, 0, 378, 160))
important_textbox.setFont(labels_font)
important_textbox.setReadOnly(True)
important_textbox.setText("- للكتابة بالبايتات في الحقول الصغيرة اكتب [b] ثم اكتب بعدها بالبايتات.\n  (ولا تضع فراغات)\n- لا تفتح ملفات الاكسل أثناء تشغيل الأداة.")
...
MainWindow.setWindowTitle("ATCEE 1.1")
...
def convert(text):
    ...
    if FIB_check.isChecked():
        if not path.exists(chars_width_database_directory):
            QMessageBox.about(MainWindow, "!!خطأ", "قاعدة بيانات عرض الحروف غير موجودة،\nتم إيقاف كل العمليات.")
            return
    ##
...
important.triggered.connect(lambda: ImportantWindow.show())
